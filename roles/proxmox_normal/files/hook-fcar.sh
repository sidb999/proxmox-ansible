...
FCAR_TMPLT=/var/lib/vz/snippets/fcar-base-tmplt.yaml
...
if [[ "${phase}" == "pre-start" ]]; then
	...
	if [[ $(qm cloudinit dump ${vmid} user | ${YQ} '.ssh_authorized_keys[]' - 2>/dev/null) ]]; then
		...
		ssh_authorized_keys="$(qm cloudinit dump ${vmid} user | ${YQ} '.ssh_authorized_keys[]' -)"
	else
		echo
...
passwd:
  users:
    - name: "${ciuser:-core}"
      ...
      ssh_authorized_keys: [ ${ssh_authorized_keys[0]} ]
